...
Copy the `pluginreview` and `sdkbox` directories from `plugin/android/jni` to your `proj.android-studio/app/jni/` directory. If the `sdkbox` folder exists, it's ok to overwrite it.

Copy the `plugin_review_res_project` directory from `plugin/android` to your `proj.android-studio` directory.
